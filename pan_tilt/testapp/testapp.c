...
//  relative to the process working directory. If you do not need network devices then "set_bindy_key" is optional.
	...
	if (!device_specified)
	{
		...
		strcpy( device_name, get_device_name( devenum, 1 ) );
	}
	...
	if ((result = command_move ( device, 50, -255 )) != result_ok)
		wprintf( L"error command_movr %ls\n", error_string( result ) );

	get_position(device, &getPosition);
	wprintf(L"Encposition: %d, uPosition: %d, Position: %d\n", getPosition.EncPosition, getPosition.uPosition, getPosition.Position);
	...
	strcpy( device_name2, get_device_name( devenum, 0 ) );
	// free_enumerate_devices( devenum );
	device2 = open_device(device_name2);

	if ((result = command_left( device2 )) != result_ok)
		wprintf( L"error command left %ls\n", error_string( result ) );

	msec_sleep( sec*1000 );
	
	if ((result = command_zero( device2 )) != result_ok)
		wprintf( L"error zeroing: %ls\n", error_string( result ) );

	if ((result = get_status( device2, &state )) != result_ok)
		wprintf( L"error getting status %ls\n", error_string( result ) );
	print_state( &state );

	if ((result = get_engine_settings( device2, &engine_settings )) != result_ok)
		wprintf( L"error getting engine settings %ls\n", error_string( result ) );

	if ((result = command_move ( device2, 50, 30 )) != result_ok)
		wprintf( L"error command_movr %ls\n", error_string( result ) );

	if ((result = command_wait_for_stop( device2, 100 )) != result_ok)
		wprintf( L"error command_wait_for_stop %ls\n", error_string( result ) );

	if ((result = command_stop( device2 )) != result_ok)
		wprintf( L"error command stop %ls\n", error_string( result ) );


	if ((result = close_device( &device2 )) != result_ok)
		wprintf( L"error closing device %ls\n", error_string( result ) );
